...
```{r}
# 1. Generamos las variantes para el escenario Cardiovascular (CVD)
# Usamos cvd_sanited porque es la versión más limpia (sin NAs y con BMI corregido)
cvd_variants <- prepare_model_variants(cvd_sanited)

cvd_cluster  <- cvd_variants$cluster
cvd_trees    <- cvd_variants$trees
cvd_logistic <- cvd_variants$logistic

# 2. Generamos las variantes para el escenario Cáncer (Breast Cancer)
# Usamos cancer_typed porque ya venía limpio de NAs y Outliers
cancer_variants <- prepare_model_variants(cancer_typed)
...
res_pca_hcv <- perform_manual_pca(cancer_cluster, "Cancer: Variancia por Componente")
