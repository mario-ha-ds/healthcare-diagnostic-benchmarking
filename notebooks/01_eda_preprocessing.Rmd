...
Nota: usaremos el csv descargable des de ahi a fecha de realizacion de esto (1/2/2026)

### B. Breast Cancer Wisconsin (Diagnostic) - UCI

Este dataset clínico, donado por la Universidad de Wisconsin, contiene mediciones morfológicas obtenidas a partir de imágenes digitalizadas de una aspiración con aguja fina (FNA) de una masa mamaria de 569 pacientes. El dataset se compone de 10 medidias que describen características geométricas y de textura de los núcleos celulares de celulas tumorales (radio, perímetro, área, concavidad, etc.) para las cuales cada una tiene 3 medidas (variables) segun media, sd i peor valor, generando un total de 30 variables numericas continuas.
...
Nota: usaremos el .data descargable des de ahi a fecha de realizacion de esto (1/2/2026)
...
Este dataset tiee 569 observaciones con 32 variables.Enntre elas se ecuentran el taregt "diagnosis" la cual indica si es malignoo o beningno. luego se encuentran tambien 10 diferentes caractersticas modfologicas observadas en el nucleo en citologias en celulas tumorales. Para cada imagen, se identifican los núcleos celulares y se computan 10 características base. Para cada una de estas características, se calculan tres estadísticos, generando las 30 variables predictoras del dataset:

Media (_mean): El valor promedio de la característica en todos los núcleos de la imagen.

Error Estándar (_sd): Mide la variabilidad técnica de la medición.

"Worst" o Peor Valor (_worst): Definido específicamente por los autores como la media de los tres valores más grandes detectados para esa característica. Este es el indicador clínico más potente, ya que el diagnóstico a menudo depende de las células más aberrantes.

| Variable | Descripción Técnica (según autores) | Tipo Analítico | Posibles Valores (Muestra) |
| --- | --- | --- | --- |
| `id` | Identificador único de la muestra | Categórica (ID) | 842302, 842517, 84300903... |
| **`diagnosis`** | **Target: Diagnóstico del tumor** | **Categórica** | **"M" (Malignant), "B" (Benign)** |
| `radius` | Media de las distancias del centro a los puntos del perímetro | Numérica | 17.99, 20.57, 19.69, 11.42... |
| `texture` | Desviación estándar de los valores de la escala de grises | Numérica | 10.38, 17.77, 21.25, 20.38... |
| `perimeter` | Longitud total del contorno del núcleo | Numérica | 122.8, 132.9, 130, 77.58... |
| `area` | Superficie total del núcleo celular | Numérica | 1001, 1326, 1203, 386.1... |
| `smoothness` | Variación local en la longitud de los radios | Numérica | 0.1184, 0.0847, 0.1096, 0.1425... |
| `compactness` | Variable derivada:  | Numérica | 0.2776, 0.0786, 0.1599, 0.2839... |
| `concavity` | Severidad de las porciones cóncavas del contorno | Numérica | 0.3001, 0.0869, 0.1974, 0.2414... |
| `concave_points` | Número de porciones cóncavas en el contorno | Numérica | 0.1471, 0.0701, 0.1279, 0.1052... |
| `symmetry` | Grado de simetría en la forma nuclear | Numérica | 0.2419, 0.1812, 0.2069, 0.2597... |
| `fractal_dimension` | Aproximación de la línea de costa () | Numérica | 0.0787, 0.0566, 0.0599, 0.0974... |
...
Nota: aunque aqui podiramos eliminar aun mas redunancias (p ej la altura o peso porque tenemos ya BMI) las mantendremos de momento para no hacer un pre-filtrado y las trataremos mas adelante ya que podrian sernos utiles antes.
...
## 1.6 Valores extremos y outliers
...
Para evitar data leakage elimnaremos momentariamente las target de cada datset, para que el algoritmo no base las respuestas en base a el. Tambvien eliminaremos cualquier factor: nos centreemos en numericas para que la metrica de distancia sea  puramente numerica.,
...
Veiem que ambdues metriques tenen inconcruencies (en especial bmi amb un ~90%). En la formula aplicada s'ha establert una tolerància d'error molt estricta (0.01). Això explica per què observem un percentatge d'error tan elevat. Si aguemntem la tolerància (per exemple, a 0.2) observem comportaments dispars: mentre que en el Waist-to-Height Ratio l'error es residualitza fins a un insignificant 0,13% —confirmant que les discrepàncies eren mers arrodoniments en la captura original—, en el BMI persisteix un 43,1% d'incongruències. Això indica la presència d'errors de càlcul o de registre que van més enllà de la precisió decimal i posa en avisque potser el dataset te baixa fidelitat!!!!
...
## 1.9 Standarization i division del dataset
...
Mantendremos el target (o ground truth) en todos los datasets praa poder vrificar. obviamente, se debran dejar siempre fuera del entrenamiento.

```{r}
# Generamos las variantes especificando la etiqueta de cada escenario
cvd_variants    <- prepare_model_variants(cvd_sanited, "cvd_risk_level")
cancer_variants <- prepare_model_variants(cancer_typed, "diagnosis")

# Asignación de datasets
cvd_cluster  <- cvd_variants$cluster
cvd_trees    <- cvd_variants$trees
cvd_logistic <- cvd_variants$logistic

cancer_cluster  <- cancer_variants$cluster
cancer_trees    <- cancer_variants$trees
cancer_logistic <- cancer_variants$logistic
```
...
- Per als datasets dels models no supervisats: Els models de clustering basats en distàncies són molt sensibles a la redundància; si dues variables diuen el mateix, el seu pes en el càlcul de la distància es duplica artificialment, esbiaixant els clústers. Per solucionar-ho, per als datasets delsno supervisats, aplicarem una Anàlisi de Components Principals (PCA). El PCA transforma les variables originals en unes de noves anomenades Components Principals, que són ortogonals entre si (no col·lineals) i capturen la màxima variància possible. Això ens permet reduir la dimensionalitat eliminant el soroll de forma automàtica.

- Per als datasets dels models supervisats: En els models supervisats, no és recomanable aplicar PCA, ja que perdríem la interpretabilitat clínica: les regles de l'arbre deixarien d'expressar-se en unitats reals (com el pes o el colesterol) per fer-ho en components abstractes i en reg logistica ... COMPLETA . Són naturalment robustos davant la redundància. Si dues variables estan molt correlacionades, l'algoritme simplement triarà la que millor separi les classes en cada tall (split), ignorant l'altra. Tot i això, mantenir redundàncies extremes pot fragmentar la importància de les variables i fer els arbres innecessàriament complexos. No obsant, la regresio logistica odia la colinealitat. s extremadament sensible a la multicolinealitat. Si incloem variables altament correlacionades, els coeficients es tornen inestables, els errors estàndard s'inflen i la capacitat del model per identificar quina variable causa realment el risc desapareix.

Por tanto, aqui, volverems a hacer ua estrategia diferenciada entre tree i entre regreson logisitca: para los de regresionaplicarem una eliminacio manual de redundancies badaes en criteris matematics (com el coef de corr) i de coneixement de domeini. Per als de arbres, pero, mantindrem totes les varaibles originals, per a que exploti tota la seva potència.

### Matriu de Correlació

Comencemos el tratamiento manual para el dataset de regresion.
...
Empecemos con el de cvd:

```{r, fig.width=10, fig.height=8}
# Llamada directa en una sola línea por dataset
plot_correlation_matrix(cvd_logistic, "CVD: Análisis de Redundancias")
```
...
- height_m i weight: al estar reconocidas en bmi
...
Pasemos ahora al de cancer:

```{r}
plot_correlation_matrix(cancer_logistic, "BBlader Cancer: Análisis de Redundancias")
```

Vemos aqui que, al ser el num d variables nuerica mucho mayor respecto al de cvd, la complejidad de la mtriz de correlacion tambien augmenta. vemos que aqui las corr, en general, se dispara: hay muchas que superan el critrrio de >85% y , fuera de eso, vemos correlaciones casi todos los posibles valores (0,7 corr, 0,6, 0,5...). Nosotros nos centraremos en las que superan el criterio ya que son las cque nos interesan

Para empezar, vemos que muchas de las correlaciones mas elevadas corresponden a metricas que representan forman parte de la msma realidad. El ejemplo mas claro es el bloque tamaño que recoje radius, perimeeter i area (en sus veriosnes mean, sd i worst). Las correlacoines son casi perfectas (como se ve claramente con perimeter_mean i raius_mean, con un valor APROXIMADO de 1, o area_mean i radius_mean, con 0.99). Lo dicho, corresponden a una misma realidad: tamaño. Es irrelevante tener el perimetro o el radio si ya tenemos el area. Por tanto, mantendremos todas las de area i eliminaremos todas las de radius i perimeter.

Por otro lado, tambien tendriamos el bloque "irregularidad". Aqui tendriamos 2 varaibles que tambien forman parte de la misma realidad: concave_points_mean i concavity_mean.Aunque mas bajoo que el grupo anteiorr, sigue teniendo alta corr (con el ejemplo kas alto: concave_points_mean vs concavity_mean (0.921)). Nos quedaremos solo con concave_points que segun la literatura del dataset oficial (Wolberg) tiene mejor capacidad predictiva que concavity

Por otro lado casi todas las variables tienen una correlación altísima entre su valor promedio y su valor máximo (el "peor" caso). Por ejemplo: radius_worst vs radius_mean (0.969). Esto ocurre con todas las variables excepto con smoothness, fractal_dimension i symmetry. Esto indica que si una célula es "rara" en promedio, sus células más extremas también lo son.  Igualmente, en cancer, nomalmente el peor (que en este caso recordemos es un promedio de las 3 peores, asique se diluye la prob de que sea un simple outleir o valor extraño) son las que suelen determinar las celulas cancerigenas. no lo hacen las buenas. Por tanto, solo en aquellas metricas donde corr >0,85, eliminaremos mean i mantendremos solo worst, que es la mas informativa en este contexto. Aun asi, solo lo harmos cuando se cumppla la condicion ya que eliminarlo sistematicaente solo sesgaria al modelo a aprender el escenario "malo" sin entender lo normal. Aun asi se perderá bastante informacion de lo "nomal" (debido a que elimiamos 7 de las 10 mean) pero el modelo es lo que requiere (y añadilo solo perjudicaria el moelo).

Asi pues, eliminemos tanto las de cvd como las de cancer, y verifiquemos que no quede ninguna con corr >0.85



```{r}
# Aplicamos la limpieza solo a las versiones logísticas
logistic_cleaned <- clean_redundancies_logistic(cvd_logistic, cancer_logistic)

# Sobreescribimos con las versiones ya sin redundancias
cvd_logistic    <- logistic_cleaned$cvd
cancer_logistic <- logistic_cleaned$cancer

plot_correlation_matrix(cvd_logistic, "cvd corregido: Análisis de Redundancias")

plot_correlation_matrix(cancer_logistic, "BBlader Cancer corregido: Análisis de Redundancias")
```

Como vemos CVD no tiene practicamente redunancias (ni correlaciones elevadas, la maxima es 0,23) lo cual es ideal para regresion. No ostante, cancer, sigue mateninedo alguna corr alta, (por ej de 0.81) aunque ninguna que supere el limte d e0.85. Aun asi, decidimos mantenernos auqi ya que metodos mas agresivos (como un intervalo de 0.75) eliminaria demasiada varianza la cual es extremadamente util en contexto clinico. Una correlación de 0.81 indica que las variables comparten un 65% de información ($R^2 \approx 0.65$), pero todavía hay un 35% de varianza única en cada una. Si sigues borrando, corres el riesgo de eliminar "señal" biológica real que el modelo podría usar para distinguir casos difíciles. La Regresión Logística es sensible, pero no tanto como para "romperse" con un 0.81.
...
Este sera el metodo que aplicaremos para reducir al minimo la colinealidad en los datasets para modelos no supervisados. 
...
Para decidir cuántas componentes conservar (cuantos k autovectores selccionamos en el paso anteiro), calculamos la Proporción de Varianza Explicada (PVE):$$\text{PVE}_{j} = \frac{\lambda_{j}}{\sum_{i=1}^{p} \lambda_{i}}$$Donde:$\text{PVE}_j$: es el porcentaje de información que retiene la componente $j$.$\lambda_j$: es el autovalor de la componente actual.$\sum \lambda_i$: es la suma de todos los autovalores (varianza total). Una regla general es escoger tantas componentes como para explicar un x % de la varainza, genralmente 85%. Aun asi, este metodo tiene limitaciones, como veremos.

Asi, lo que hraemos sera calcular las pca i represntar en un scree plt la variabildiad ed cada componente, junto con una linea curva qiue represente la varianza acumulada. Est  nospermitira escoger el num de componentes ideal.
...
res_pca_cancer <- perform_manual_pca(cancer_cluster, "Cancer: Variancia por Componente")
```
Los graficos corroboran nuestra hipotesis inicial: parece qe en cvd muchas de las variables son importantes i no hay solo factor (o unos pocos) determinantes. De las 12 variables que comprenden al dataset, se necesitan 7 para explicar el anisiado 85% (aprox ya que relamente es 84,61) de la variabilidad, y 9 para explciar aproximadamente el 100%. Y ademas, la redccion de varaibilidad es menos notoria: al principio la dismninucion de la variabiliadd no es exageera i de hecho, de la 5a a la 9a coponente todas explican casi lo mismo, sin disminucion significativa. Estyo ultimo indica claramente que el dataset es isotrópico (la información está repartida en muchas direcciones con la misma fuerza) Esto refuerza la idea de el riesgo cvd como un hecho multifactorial i suma puntos a nuesra hipotesis de continuum: al necesitar muchas vareaibles, la diferencia entre riesgo alto o bajo, estara difuminada ya que se puede ser alto segun un factor pero bajo segun otro. 
Es curioso ademas, que justo 7 componentes (~85%) es identico a el numero de variables que hemos mantenido nosotros en el apartado anterior, al reducir manualmente rednndancias. Esto, seguramente, no es casual: seguramente hemos seleccionado correctamente las variables que eran ruido o repetidas, ca`ptando las mismas componentes que la pca ha hecho, aunque con otro metodo.
E cualquier caso, que hacemos en este caso? nos queamos con 7 componentes? Bueno, asumir simplemente el umbral tiene sus limitaciones. Aunque  es una regla que generalmente funciona, ya que nos permite seleccionar la mayor variabilidad i cantidad de info en el menor num de variables, tabien hace que podamos perder indormaicon relevante. En este4 caso, vemos que de la 7a a la 8a componente practicamente la variabilidad es la inmsma, i de la 8a a la 9a igual. de manera que todas ellas son igualmente relevantes. selecionar la 7a i no la 8an sera injusto ynos haria perfder informacion relevante, i mas en el reisgo cvd donde cada informacion importa. Por tanto, aqiuo, decidiremos obviar el umbral i escoger 9 componentes, las suficientes para epliar cerca del 99% de la variabilidad, i eñ punro real donde hay un wlbow en la bvarianza acu,mulada. Eliminamos 3 componentes que son el ruido real i, teniendo en cuenta nuestra dea de continuum, aseguramos no perder info relevante.

El contraste con el dataset de cáncer es notable. A pesar de partir de 30 variables numéricas, el sistema colapsa drásticamente la información: solo 5 componentes bastan para explicar el 85% (realemnte 84.73%) de la variabilidad. Aqui, ademas, la disminucion de la variabiliadd tras cada componente es mucho mas marcada, remarcando la idea de que las primaras son mucho mas importantes. Las primeras dos compentes explican la gran matoria de la varibilidad, al contrairo de cvd donde era todo mas equitativo.
Este resultado es la prueba matemática de lo que observamos en la matriz de correlación: la mayoría de las 30 variables originales eran "ecos" geométricos unas de otras (radios, perímetros, áreas). La PCA ha filtrado ese ruido, demostrando que la "huella dactilar" de la malignidad se puede resumir en apenas 5 dimensiones. A diferencia de CVD, aquí la varianza se concentra en las primeras componentes. Esto suele ser un indicador de que los datos tienen una estructura interna muy fuerte y linealmente separable, lo que apoya nuestra hipótesis de que encontraremos grupos (Benigno vs. Maligno) con fronteras mucho más definidas.
Aqui es mucho mas dificil ver el elvbow. la varianza acumulada se cmporta de forma logaritmica, al principoi crece muchoi (por la gran varianza de las primeras componentes) pero progresivamente se va aplcanadno, debido a que cada vez explcian menos las componentes. Aqiu, el la componente que exzplica aprox el 85%, la 5a, el salto a la siguiente componente (6a) es mucho menos marcada que en el caso anterior. realmente la ganancia de varivilidad es muy peqeueña si se compara con las que ya se tienen. i en cuantas mas se añaden, mas marcadoo es. Se necesitarian cerca de 19 componnentes para explciar esa aprox 99% . Por tanto, aqui, cremos que es mas coinveniente cofiar en las primeras i seleccionar el minimo ruido posible: 5 componentes.

Para entender qué hay detrás de estas componentes, debemos fijarnos en los Loadings (o autovectores, $w$).Cada componente principal es una combinación lineal de las variables originales, donde cada variable tiene asignado un peso específico. Estos pesos representan la correlación entre la variable original y la componente: cuanto mayor es el valor absoluto del peso, mayor es la influencia de esa variable en la definición de ese eje.En términos matemáticos:Los Autovalores ($\lambda$) nos dicen cuánta información captura la componente (la varianza).Los Autovectores ($w$) o pesos nos dicen de qué está hecha esa información, permitiéndonos identificar qué variables son las más determinantes para cada factor.

Asi, ara entender cada componente seleccionada, buscaremos para cada componetne, los pesos de las 4 variaables con mas peso en cada componetne, empezando por cvd:

```{r}
# 1. Análisis de pesos para CVD (9 componentes)
top_cvd <- get_top_features_pca(res_pca_cvd, n_pcs = 9, top_n = 4)

print(top_cvd)
```
PC1 (Eje Metabólico-Antropométrico): Representa la obesidad central. El peso máximo lo tienen el ratio cintura-altura y el BMI. Es el factor de forma física.

PC2 (Eje Lipídico): Representa el colesterol. Está dominado por el LDL y el colesterol total. Curiosamente, tiene una relación inversa con el peso, lo que sugiere que hay perfiles de riesgo lipídico que no dependen solo de la obesidad.

PC3 (Eje de Composición Corporal): Funciona como un ajuste del PC1, refinando la relación entre el peso total y el perímetro abdominal para distinguir entre diferentes tipos de acumulación de grasa.

PC4 (Eje Metabólico-Edad): Relaciona el azúcar en sangre con el envejecimiento y la presión sistólica. Es el eje del desgaste metabólico.

PC5 y PC6 (Eje Protector/Físico): Introducen el HDL (colesterol bueno) y la altura. El hecho de que el factor protector (HDL) aparezca en ejes separados refuerza su rol independiente frente a los factores de riesgo de las primeras componentes. 

PC7 y PC8 (Eje de Hipertensión): Se centran casi exclusivamente en la presión arterial (diastólica y sistólica). Que necesiten sus propias componentes indica que la tensión va por un camino distinto al colesterol o al peso.

PC9 (Eje de Glucemia): Dominado por el azúcar en sangre y la edad. Similar en cuanto a vairables al 4.

La arquitectura de estas componentes demuestra que el riesgo es un rompecabezas de 9 piezas ortogonales. Aunque existen solapamientos y repeticiones en las variables que las componen, el hecho de que la varianza se reparta de forma tan específica valida nuestra hipótesis del continuum: un paciente puede presentar un riesgo alto en el eje lipídico (PC2) pero estar sano en el eje hemodinámico (PC7), lo que difumina las fronteras entre grupos de riesgo y justifica un análisis más detallado que el simple binomio "sano/enfermo".
El predominio de la composición corporal y el perfil lipídico en las primeras componentes (las que capturan más varianza) sugiere que estos son los motores determinantes de la variabilidad en la población estudiada.

Pasemos ahora al segundo dataset:

```{r}
# 1. Análisis de pesos para Cancer (5 componentes)
top_cancer <- get_top_features_pca(res_pca_cancer, n_pcs = 5, top_n = 4)
print(top_cancer)
```
Aquí el comportamiento es el de una "máquina" donde las piezas encajan de forma muy precisa.

PC1 (Eje de Agresividad Morfológica): Es la componente más importante. Representa la irregularidad y complejidad del tumor (puntos cóncavos y concavidad). Si esta componente es alta, el tumor es morfológicamente sospechoso.

PC2 (Eje de Complejidad Fractal): Se centra en la auto-similitud del borde del tumor. Un crecimiento fractal desordenado es típico de procesos malignos.

PC3 (Eje de Inestabilidad/Textura): Mide la desviación estándar (SD) de la textura y la simetría. Representa qué tan "caótico" o heterogéneo es el tejido por dentro.

PC4 (Eje de Apariencia Visual (Textura)): Dominado totalmente por la textura (la "rugosidad" de la imagen en la mamografía).

PC5 (Eje de Suavidad): : Representa la suavidad y uniformidad. Funciona como un contrapunto a los ejes de irregularidad, midiendo qué tan "limpios" son los límites de la masa celular.

Mientras que en CVD el riesgo era un puzle de piezas independientes, en el cáncer la información es jerárquica. La $PC1$ y la $PC2$ por sí solas ya cuentan la mayor parte de la historia. Esta concentración de la varianza en solo 5 o 6 ejes sugiere que estamos ante un problema con fronteras biológicas muy marcadas.

Conociendo ya que dinica cada copnente, estableceremos dlos datasets finales para clustering: con 9 componentes para cvd y 5 para cancer.

```{r}
# Generamos el dataset final para el escenario Cardiovascular (9 componentes)
cvd_final_pca <- generate_pca_datasets(
  pca_results = res_pca_cvd, 
  df_original = cvd_cluster, 
  target_col  = "cvd_risk_level", 
  n_pcs       = 9
)

# Generamos el dataset final para el escenario de Cáncer (5 componentes)
cancer_final_pca <- generate_pca_datasets(
  pca_results = res_pca_cancer, 
  df_original = cancer_cluster, 
...
Llegamos a este punto ya tenemos tanto los datasets _clustering, -logistic i _trees preparados, cada uno con sus caracteristicas especificas.distribuciones de numaercias segun target, entre otroxs
